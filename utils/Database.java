...
public class Database {
    ...
    private static final String FLIGHT_DATA_FILE = "flights.json";

    private static final String BOOKING_DATA_FILE = "bookings.json";
    ...
    public static void saveFlightsToFile() {
        try (Writer writer = new FileWriter(FLIGHT_DATA_FILE)) {
            gson.toJson(flights, writer);
            System.out.println("✅ Flights saved to flights.json");
        } catch (IOException e) {
    ...
    public static void loadFlightsFromFile() {
        ...
        try (Reader reader = new FileReader(file)) {
            ...
            System.out.println("✅ Loaded " + flightCount + " flights from file.");
        } catch (IOException e) {
            System.out.println("❌ Error loading flights: " + e.getMessage());
        }
    }
    public static void saveBookingsToFile() {
        try (Writer writer = new FileWriter(BOOKING_DATA_FILE)) {
            gson.toJson(bookings, writer);
            System.out.println("✅ Bookings saved to bookings.json");
        } catch (IOException e) {
    ...
    public static void loadBookingsFromFile() {
        File file = new File(BOOKING_DATA_FILE);
        if (!file.exists()) return;

        try (Reader reader = new FileReader(file)) {
            bookings = gson.fromJson(reader, new TypeToken<List<Booking>>() {}.getType());
            bookingCount = bookings.size();
            System.out.println("✅ Loaded " + bookingCount + " bookings from file.");
        } catch (IOException e) {
